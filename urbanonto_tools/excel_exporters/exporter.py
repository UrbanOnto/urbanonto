...
def __export_row(index, row, ontology: Graph, ontology_with_imports: Graph, entity: URIRef):
    ...
    if index == FUNCTION_ROW_NO:
        if len(pl_row_value) > 0:
            __add_functions_to_entity(functions_string=pl_row_value, entity=entity, ontology=ontology, ontology_with_imports=ontology_with_imports)
    ...
    if index == SYNONYM_PL_ROW_NO:
        if len(pl_row_value) > 0:
            __split_cell_into_literals(cell=pl_row_value, subject=entity, predicate=SKOS.altLabel, lang='pl',
                                       ontology=ontology)
...
def __add_functions_to_entity(functions_string:str, entity: URIRef, ontology: Graph, ontology_with_imports: Graph):
